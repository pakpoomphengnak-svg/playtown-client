// client/js/weapon/poolcue5.js
// ────────────────────────────────────────────────────────────
// Weapon Definition: ไม้พลู +5 (Pool Cue +5) 🎱
// อาวุธอัปเกรดสูงสุดจากไม้พลู — สีแดงเรืองแสง
// ต้องโหลดหลัง js/system/weapon.js
// ────────────────────────────────────────────────────────────

WEAPON_DEFS.poolcue5 = {
  id:          'poolcue5',
  name:        'ไม้พลู +5',
  image:       'assets/weapons/poolcue5.png',
  emoji:       '🎱',
  description: '',
  maxStack:    1,
  ...
  // เหมือนโครงไม้พลูเดิม แต่เปลี่ยนวัสดุเป็นโทนแดงเรืองแสง
  createModel() {
    const group = new THREE.Group();

    const C_BUTT  = 0x2e0000; // ด้ามจับ (แดงเข้มเกือบดำ)
    const C_WRAP  = 0x1a0000; // แถบคาดเข้ม (ดำอมแดง)
    const C_SHAFT = 0xef5350; // เพลาไม้ (แดงสด)
    const C_TIP   = 0xff8a80; // ปลายคิว (แดงสว่างกว่าเพลา)
    ...
    for (const target of targets) {
      if (typeof target.takeDamage === 'function') {
        ...
        if (typeof Inventory !== 'undefined' && typeof Inventory._toast === 'function') {
          if (isCrit) {
            Inventory._toast(`คริติคอล! ตีด้วยไม้พลู +5 -${dmg} HP 💥`, { icon: '🎱', color: '#ff1744' });
          } else {
            Inventory._toast(`ตีด้วยไม้พลู +5! -${dmg} HP`, { icon: '🎱', color: '#ef5350' });
          }
        }
      }
    }
...
if (typeof ITEM_DEFS !== 'undefined') {
  ITEM_DEFS.poolcue5 = WEAPON_DEFS.poolcue5;
}
