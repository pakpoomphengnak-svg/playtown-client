// client/js/weapon/bottle5.js
// ────────────────────────────────────────────────────────────
// Weapon Definition: ปากฉลาม +5 🍾
// อาวุธอัปเกรดสูงสุดจากปากฉลาม — สีแดงเรืองแสง
...
WEAPON_DEFS.bottle5 = {
  id:          'bottle5',
  name:        'ปากฉลาม +5',
  image:       'assets/weapons/bottle5.png',
  emoji:       '🍾',
  description: '',
  maxStack:    1,
  ...
  // เหมือนโครงปากฉลามเดิม แต่เปลี่ยนวัสดุเป็นโทนแดงเรืองแสง
  createModel() {
    const group = new THREE.Group();

    const C_CAP    = 0x330a0a; // ฝาขวด (แดงเข้มเกือบดำ)
    const C_NECK   = 0xb71c1c; // คอขวด (แดงเข้ม)
    const C_BODY   = 0xef5350; // ลำตัวขวด (แดงสด)
    const C_BASE   = 0xb71c1c; // ก้นขวด (แดงเข้มกว่านิด)
    const C_CORE   = 0xff8a80; // แกนเรืองแสง (แดงสว่างจัด)
  ...
  onAttack(attacker, targets) {
    ...
    if (hit && typeof Inventory !== 'undefined' && typeof Inventory._toast === 'function') {
      Inventory._toast(`ฟาดด้วยปากฉลาม +5! -${this.damage} HP`, { icon: '🍾', color: '#ef5350' });
    }
...
if (typeof ITEM_DEFS !== 'undefined') {
  ITEM_DEFS.bottle5 = WEAPON_DEFS.bottle5;
}
